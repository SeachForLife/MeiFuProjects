...
public class FavoriteAdapter extends BaseRecyclerAdapter<FavoriteAdapter.FavoriteAdapterViewHolder> implements View.OnClickListener {
    ...
    public FavoriteAdapter(List<PersonCollect> list, Context context) {
    ...
    public void setData(List<PersonCollect> list, Context context) {
        this.personCollectList = list;
        this.myContext = context;
    ...
    public void insert(PersonCollect pc, int position) {
    ...
    public void onBindViewHolder(FavoriteAdapterViewHolder holder, int position, boolean isItem) {
        PersonCollect pc = personCollectList.get(position);
        String load_url = "";
        String url = "";
        if (pc.getGet_type() == 0) {
            url = ArtTeachiingApplication.imageUrlFront;
        } else {
            url = ArtTeachiingApplication.localUPversion+"ART/";
        }
        if (pc.getType().equals("1")) {
            load_url = url + pc.getP_url() + "";
            holder.iconVideo.setVisibility(View.VISIBLE);
            holder.iconVideo.bringToFront();
        } else {
            holder.iconVideo.setVisibility(View.GONE);
            load_url = url + pc.getP_url() + "_320.jpg";
        }
